...
double ComputeInitialScore()
{
  ...
  for (m=0 ; m<Layers ; m++)
      for (i=0 ; i<MaxComms ; i++)
      {
         ...
         BestCommStubsNew[i] += BestCommStubs[m][m][i];
         for (j=0 ; j<MaxComms ; j++)
             BestEdgeMatrixNew[i][j] += BestEdgeMatrix[m][m][i][j];
      }
                         ...
                         else
                         {
                             if (BestCommVerticesNew[i] != 0)
                             {
                                //cout<<"kenari3"<<endl;
                                  ...
                                  sum = sum - (double(BestCommStubsNew[i]) ) * log(double(BestCommVerticesNew[ i]) );
                             }
                         }   
                          ...
                          if (DegreeCorrect == 1)
                           {
                                      ...
                                      sum = sum - LogFunction(double(BestCommStubs[m][l] [i]+BestCommStubs[l][m] [i]) ) ;
                                       //sum = sum - LogFunction(double(BestCommStubs[m][l] [i]) );
                              ...
                              { 
                                       //cout<<"kenari4"<<endl;
                                        ...
                                        //sum = sum + LogFunction(double(BestEdgeMatrix[m][l][i][j]) );
                              }
...
double ComputeProposal(int vertex, int azz, int destination,int fromLayer)
{
  ...
  for (m=0 ; m<Layers ; m++)
      for (i=0 ; i<MaxComms ; i++)
      {
         ...
         CurrentCommStubsNew[i] += CurrentCommStubs[m][m][i];
         for (j=0 ; j<MaxComms ; j++)
             CurrentEdgeMatrixNew[i][j] += CurrentEdgeMatrix[m][m][i][j];
      }
                 ...
                 if (NeighborLayers[ l] == 1)
                 {
                    ...
                    for (i=0 ; i<ActualDiffComms[l] ; i++)
                    {
                           ...
                           if ((NeighborSet[ l ] [0 ][ i] != azz) && (NeighborSet[ l][0 ][i] != destination))
                           {
                               ...
                               {
                                    help1 = double(CurrentEdgeMatrixNew[azz ][NeighborSet[l ][0 ][i]]);
                                    help2 = double(CurrentEdgeMatrixNew[destination ][NeighborSet[l ][0 ][i]]);
                                    help3 = double(NeighborSet[l ][1 ][i]);
                                    ...
                                    help1 = double(CurrentEdgeMatrixNew[NeighborSet[l ][0 ][i]][azz ]);
                                    help2 = double(CurrentEdgeMatrixNew[NeighborSet[l ][0 ][i]][destination ]);
                                    help3 = double(NeighborSet[l ][1 ][i]);
