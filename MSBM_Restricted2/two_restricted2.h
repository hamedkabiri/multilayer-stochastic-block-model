...
int BestCommVerticesNew[MaxComms];
int BestCommStubsNew[MaxComms];
int BestEdgeMatrixNew[MaxComms][MaxComms];

int CurrentCommVerticesNew[MaxComms];
int CurrentCommStubsNew[MaxComms];
int CurrentEdgeMatrixNew[MaxComms][MaxComms];
...
int TempNeighborSetNew[2][MaxComms];  // the first entry lists the comm and the second entry lists the number of edges to that comm
int NeighborSetNew[2][MaxComms]; // this is what we record and use
...
double ComputeInitialScore()
{
  ...
  for (m=0 ; m<Layers ; m++)
    for (l=0 ; l<Layers ; l++)
      for (i=0 ; i<MaxComms ; i++)
      {
         BestCommVerticesNew[i] += BestCommVertices[m][i];
         if (m!=l)
         {
            BestCommStubsNew[i] += BestCommStubs[m][l][i];
            for (j=0 ; j<MaxComms ; j++)
                  BestEdgeMatrixNew[i][j] += BestEdgeMatrix[m][l][i][j];
         }
      }
        ...
        for (i=0 ; i<MaxComms ; i++)
                  ...
                  for (j=0 ; j<MaxComms ; j++)
                  {
                          ...
                          for (m=0 ; m<Layers ; m++)
                                    ...
                                    sum = sum + LogFunction(double(BestEdgeMatrix[m][m][i][j]));
                  ...
                  for (m=0 ; m<Layers ; m++)
                          for (l=0 ; l<Layers ; l++)
                          
                              {
                                  ...
                                  if (DegreeCorrect == 0)
                                     if (m!=l)
                                        if (BestCommVerticesNew[ i] != 0)
                                             sum = sum - (double(BestCommStubs[m][l][i]) ) * log(BestCommVerticesNew[ i]);
  ...
  for (m=0 ; m<Layers ; m++)
    for (l=0 ; l<Layers ; l++)
      for (i=0 ; i<MaxComms ; i++)
      {
         ...
         if (m!=l)
         {
            CurrentCommStubsNew[i] += CurrentCommStubs[m][l][i];
            for (j=0 ; j<MaxComms ; j++)
                  CurrentEdgeMatrixNew[i][j] += CurrentEdgeMatrix[m][l][i][j];
         }
      }
       ...
       long deg = 0;
        ...
        for (i=0 ; i<ActualDiffCommsNew ; i++)
        {
                   ...
                   if (NeighborSetNew[0][i] != azz && NeighborSetNew[0][i] != destination)
                   {
                        help1 = double(CurrentEdgeMatrixNew[azz][NeighborSetNew[0][i]]);
                        help2 = double(CurrentEdgeMatrixNew[destination][NeighborSetNew[0][i]]);
                        help3 = double(NeighborSetNew[1][i]);
                        ...
                        help1 = double(CurrentEdgeMatrixNew[NeighborSetNew[0][i]][azz]);
                        help2 = double(CurrentEdgeMatrixNew[NeighborSetNew[0][i]][destination]);
                        help3 = double(NeighborSetNew[1][i]);
                   ...
                   if (NeighborSetNew[0][i] == azz)
                      
                           fromcount = NeighborSetNew[1][ i];

                   if (NeighborSetNew[0][i] == destination)
                       
                            destcount = NeighborSetNew[1][ i];
        }
        for (i=0 ; i<ActualDiffComms[fromLayer] ; i++) 
        {
                      ...
                      if (int(NeighborSet[fromLayer][0][i]) != azz && int(NeighborSet[fromLayer][0][i]) != destination)
                      {
                      ...
                      if (NeighborSet[fromLayer][0][i] == azz)
                           ...
                           fromcount1 = NeighborSet[fromLayer][1][i];

                      if (NeighborSet[fromLayer][0][i] == destination)
                         
                           destcount1 = NeighborSet[fromLayer][1][i];
        }
      ...
      //now we add in the term corresponding to from / dest
        ...
        help2 = double(fromcount1 - destcount1);
        ...
        help2 = double(SelfEdgeCounter + 2 * fromcount1);
        ...
        help2 = double(SelfEdgeCounter + 2 * destcount1);
                    ...
                    if (l != fromLayer)
                        {
                             ...
                             if (help1 - help2 != 0)
                                   ratiovalue = ratiovalue - ((help1 - help2) ) * log(double(CurrentCommVerticesNew[azz] - 1));
                                 

                             if (help1 != 0)
                                   ratiovalue = ratiovalue + ((help1 )) * log(double(CurrentCommVerticesNew[azz]) );
                        }
                        else
                        {
                            help1 = double(CurrentCommStubs[fromLayer][l][azz]);
                       ...
                       if (l != fromLayer)
                         {
                              ...
                              if (help1 - help2 != 0)
                                    ratiovalue = ratiovalue - ((help1 + help2))  * log(double(CurrentCommVerticesNew[destination] + 1) );
                          


                              if (help1 != 0)
                                    ratiovalue = ratiovalue + ((help1 )) * log(double(CurrentCommVerticesNew[destination]) );
                          }
                        else
                        {
                                help1 = double(CurrentCommStubs[fromLayer][l][destination]);
...
{
  unsigned int i,j,l,m,z;//-Wunused,j;
  int neighbor;
  
  SelfEdgeCounter = 0;
  ActualDiffCommsNew = 0;
  ...
  for (j=0 ; j<MaxComms ; j++)
        {
               ...
               NeighborSetNew[0][j] = j;
               NeighborSetNew[1][j] = 0;
        }
  ...
  for (j=0 ; j<MaxComms ; j++)
          {
                ...
                TempNeighborSetNew[0][j]=j;
                TempNeighborSetNew[1][j]=0;
          }
                ...
                if (i != vertex)
                    {
                        if (option == 0)
                              ...
                              TempNeighborSetNew[ 1 ][ CurrentState[i]] += AdjMatrix[vertex][ i];
                        if (option == 1)


                              TempNeighborSetNew[1 ][BestState[i]] += AdjMatrix[vertex][i];
                    }
            } 
        ...
        ActualDiffCommsNew = 0;
                ...
                for (i=0 ; i<MaxComms ; i++)
                    if (TempNeighborSetNew[1][i] !=0)
                    {
                       
                        NeighborSetNew[0][ActualDiffCommsNew] = TempNeighborSetNew[0][i];
                        NeighborSetNew[1][ActualDiffCommsNew] = TempNeighborSetNew[1][i];
                        ActualDiffCommsNew +=1;
                    }
  return;
}
